...
def test_due_task_ids_respects_interval_and_daily_time(tmp_path: Path) -> None:
    ...
    assert due_task_ids(cfg, store, now=now) == [TASK_WECHAT_FETCH, TASK_TUSHARE_SYNC]
...
def test_run_due_tasks_records_success_without_real_network(tmp_path: Path) -> None:
    ...
    assert [item.task_id for item in runs] == [TASK_WECHAT_FETCH, TASK_TUSHARE_SYNC]
    assert all(item.ok for item in runs)
    assert [item.last_status for item in status] == ["success", "success"]
    assert [item.last_message for item in status] == ["wechat ok", "market ok"]
